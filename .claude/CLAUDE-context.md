...
| Trigger | Paths |
|---|---|
| **Money moving** | `app/api/webhooks/stripe/`, `src/adapters/stripe-payment.ts`, `src/ports/payment.ts`, `src/reservations/booking-webhook.ts`, `src/reservations/create-departure-payment-intent.ts`, `src/reservations/create-balance-checkout.ts`, `src/reservations/payment-config.ts`, `src/reservations/refund-payment.ts`, `src/reservations/refund-terms.ts`, `src/reservations/cancel-reservation.ts`, `app/(admin)/admin/calendar/[reservationId]/actions.ts` |
...
**`src/admin/payroll.ts` and `src/admin/gratuity-payroll.ts` joined at 15.20**, and they are the fourth instance of the identical mistake — every path in the Money-computed row was under `app/`, so the two modules that *compute the numbers* matched nothing while the routes wrapping them matched. `tips.csv/route.ts` is eleven lines that call `buildGratuityPayroll` and stream the result; the split, the remainder distribution and the crew denominator are all one layer down. Found while writing 15.20's coverage for that denominator, where dropping one clause silently pays a trainee an equal share of the tip pool and shrinks every working crew member's cut — a defect entirely inside the computing half, with no route change anywhere in the diff.
