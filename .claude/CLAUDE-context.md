...
| In a worktree, `npm run db:up` fails and `npm run db:migrate` is mandatory | `docker-compose.yml` pins a fixed `container_name`, so a second compose project cannot start its own Postgres and dies on a name conflict — one container serves every database on 5432. And each worktree can carry its own `DATABASE_URL`: `muster-s91`'s pointed at a `muster_s91` database six migrations behind, which fails as `column "vessel_id" of relation "reservations" does not exist` and reads exactly like a code bug. Check with `psql "$DATABASE_URL" -c "select count(*), max(filename) from _migrations"` against `ls db/migrations/*.sql \| wc -l` |
