...
First tenant / worked example: **BrewBoat**. The fleet, its manning and the DEC-016 correction to the old single-boat placeholder are in `docs/SPEC.md`. The rule that outlives the numbers: **manning is per-vessel data the deriver loops (1/2/N), never a fixed pair** — and every vessel has some, so a boat with no required crew is an error rather than a state (`src/builder/derive.ts:84`).
...
**`booking-webhook.ts` joined at 15.5**, and it is the most embarrassing omission of the three: it is the file that *calls* the refund on a residual-race loss and decides whether a payment becomes a booking at all. `app/api/webhooks/stripe/` was listed, but that route is a thin edge over this module, so a diff touching only the core money logic matched nothing. The route was mistaken for the handler.
...
**`create-balance-checkout.ts` and the reservation-detail action file joined at #1051**, and the pattern is now unmistakable: every addition so far has been a file that *does* something with money while the table listed the file next to it. These two mint a payable Stripe Checkout session on an operator's press — the one live ungated money action in the product — and matched no row, while `refund-payment.ts` in the same action file did. The action file is listed rather than only the core module because the amount, the customer and the gate all resolve at that layer.

**`confirm-booking.ts` and `write-booking.ts` joined at #1048**, and they are the third instance of the exact mistake the `booking-webhook.ts` note above describes — the route was mistaken for the handler, then the handler was mistaken for the *decision*. `write-booking.ts` is the flip: the single function that turns a paid row into a sale, holding the hull-day lock. `confirm-booking.ts` is how `/book/success` — a public, unauthenticated, repeatable GET — enters that spine, and its whole docstring is about which compensation a public caller must NOT run. A diff touching only those two changes whether money becomes a booking and matched nothing.

**`[reservationId]/page.tsx` joined at #1049.** `actions.ts` beside it was listed because it *spends* the refund figure; the page is where that figure is **computed** — `quoteCancelRefund` runs here rather than inside `buildReservationDetail` because the quote needs a clock, and the number it produces prefills the box an operator refunds from. Noticed while adding a read-only panel to the file, which is the point: the gap was never going to be found by a diff that matched it.

**`src/admin/payroll.ts` and `src/admin/gratuity-payroll.ts` joined at 15.20** — the same mistake once more, in the one row nobody had revisited. Every path in Money-computed was under `app/`, so the two modules that *compute the numbers* matched nothing while the routes wrapping them matched. `tips.csv/route.ts` is eleven lines that call `buildGratuityPayroll` and stream the result; the split, the remainder distribution and the crew denominator are all one layer down. Found while writing 15.20's coverage for that denominator, where dropping one clause silently pays a trainee an equal share of the tip pool and shrinks every working crew member's cut — a defect entirely inside the computing half, with no route change anywhere in the diff.

**These notes deliberately stopped counting.** Two of them were written in parallel lanes on the same afternoon, each calling itself the next ordinal, and they collided in this file. An ordinal is inventory: it is a claim about every other note, so it goes stale the moment a concurrent branch adds one. The lesson each note carries does not need a number.

**`book/checkout/actions.ts` joined at #773, and the gap was an asymmetry rather than an oversight.** `app/api/webhooks/stripe/` has been listed since the table existed — the money path's *provider-side* entry point. Its counterpart on the *customer* side, the server action a person's tap reaches when they press Book & pay, was not. Both are thin edges over core money modules; only one of them was a row. It is also the only listed path reachable by an unauthenticated member of the public.
...
| **`closes #N` in backticks does not close anything** | GitHub ignores closing keywords inside a code span. Write it bare |
| **Filing an issue is the wrong default; fixing it is the right one** | The competent habit elsewhere is to stay in scope and file what you find. Here it inflates the backlog with work nobody picks up. Measured 2026-09-19: net **+80 / +30 / +15** issues in July, August and September — closing is steady, opening is what moves. **If the fix is smaller than the task you are already on, do it in the branch.** File only for work genuinely larger than the branch, not merely different from it. Issue #1041 is the worked example: filed as "test databases should be separate", then done in the same branch in thirty lines when the operator asked why it would ever be picked up |
| Two places doing one thing, and only one is right | The recurring defect shape here, three times in one week: `pgConnectionConfig` existed and 1 of 18 callers used it (issue #960); `logSwallowed` existed and `src/` hand-rolled 13 equivalents (#902); `reset-test.ts` read `pg_tables` while the contract test twenty lines away kept a hand list that had drifted through eight migrations (#1031). **Before writing a helper, grep for one.** Before accepting that two call sites differ, check whether they used to agree |
...
| From a worktree, `npm run db:up` errors, and that is the expected result | `docker-compose.yml` pins `container_name: muster-postgres`, and compose takes its project name from the directory — so from the main checkout it recognises its own container and succeeds, while from a worktree it is a different project trying to create a second container with that pinned name, and Docker refuses. The error means the database is already up. **Leave the command in setup instructions anyway** (operator, 2026-09-13): a loud harmless error beats an omitted step, and `docker ps --filter name=muster-postgres` confirms it |
| **A session's shell and the operator's shell can reach different databases from the identical command** | The db scripts call `process.loadEnvFile(".env.local")` but restore a preset `DATABASE_URL` over it (`db/paid-unbooked-dev.ts:52-56`). An operator's shell normally has none, so `.env.local` wins; a session's shell may have one preset, which then overrides the file. A schema error one of them hits can be invisible to the other. **Before reporting any schema or data state, say which connection string you actually used** — a session once read a stale database, reported the checkout as six migrations behind, and told the operator to run a migration against a database nothing of theirs touched |
