  ...
  return (
    <Shell>
      ...
      {/* Redirect-param feedback reads safely when stale ("last action", not
          "just happened") — the no-client-JS tradeoff, noted in DEC-026 era
          review. */}
      {sp.leaned && (
        <Notice tone="ok">
          ↗ Last action: leaned on {sp.leaned} — asked, not yet filled.
        </Notice>
      )}
      {sp.lean_error && <Notice tone="bad">{sp.lean_error}</Notice>}
...
async function buildVMs(rows: AtRiskRow[]): Promise<RiskRowVM[]> {
  ...
  return rows.map((r) => {
    ...
    return {
      ...
      tight: r.hoursToTrip !== null && r.hoursToTrip < 36,
      flag,
