...
export default async function CheckoutPage({ searchParams }: { searchParams: Promise<Search> }) {
  ...
  //
  // **Still no `asOf` here, deliberately, after #620 added it to /book.** Without it the deriver
  // treats no `pending` row as live, so this render ignores in-flight checkouts entirely — and it
  // must. The diagnosis on that issue named both call sites; its acceptance named only `/book`,
  // and this one should stay as it is.
  //
  // The reason survived 14.7 changing what occupies a slot. It used to be that a `CheckoutHold`
  // carried no customer identity, so there was no way to tell YOUR hold from a rival's at render
  // time. A `pending` row DOES carry `holderToken`, but `deriveVirtualAvailability` takes no
  // exemption argument — it is a pure read model with no notion of who is asking. Pass `asOf`
  // here and anyone who backs into this page while holding the slot is told it is sold out, by
  // their own row. `claimDepartureSlot` (which does exempt by token) remains authoritative, and
  // the write CAS is the real backstop either way.
