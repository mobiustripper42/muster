 ...
 *
 * **The core has its own, at `src/log.ts` (#902).** Identical contract, pinned by
 * the same five test cases, because `src/` is framework-free (DEC-013/DEC-020) and
 * cannot import from `app/` — two functions by necessity, and the paired tests are
 * what stop that becoming two behaviours. Said here because this is the file someone
 * lands in when they ask "did we fix the swallowed errors": the answer is now yes in
 * both layers, and **`db/` is deliberately excluded** — those are scripts run at a
 * terminal where an unhandled throw is already on screen.
 *
