 ...
 *      closing Dov's open punch always clears its block, while a missing day may simply be
 *      true. Gil is also the only row carrying both an estimate and actual hours.
 *
 ...
 *
 * Ada through Fen hold no seats, so their rows are clock-only: actual hours against a zero
 * estimate. Only Gil has both, so his is the only delta on the page that means anything. These
 * seven are also the only rows in the seeded period — `db:seed:crew`'s roster is seated in the
 * current and future periods, not this one.
 *
...
/**
 * A required Confirmed seat on a one-trip shift — the ESTIMATE side (#638). Everything else in
 * this seed is punches with no seats; this is the only fixture that puts a scheduled shift and
 * a person on the same vessel-local day, which is what a missing punch is measured against.
 */
async function seat(
  key: string,
  ...
  const vesselId = asId<"VesselId">("vessel-tc");
  const eventId = asId<"EventId">(`evt-tc-${key}`);
  const shiftId = asId<"ShiftId">(`shift-tc-${key}`);
...
try {
  ...
  // G — the missing punch (#638). Gil holds two required Confirmed seats. He punched one day
  // and not the other, so exactly one day is missing — which also proves the check compares
  // DAYS rather than deciding per person. His row is the only one showing both halves of the
  // union: a real estimate AND real actual hours, with a negative delta for the day he didn't
  // punch. It must NOT block the export; that's the whole asymmetry with Dov's open punch.
  const gil = await crew("crew-tc-gil", "Gil", gusto("Gil", "Gap", "GU-1007"));
  await seat("gil-punched", gil, 11, "09:00");
  ...
  console.log("SEVEN CREW, one concern each. Ada-Fen hold no seats, so they show actual hours");
  console.log("against a zero estimate. Only Gil has both, so his is the only delta that means");
  ...
  console.log("          Dov's open punch. Gil is also the only row with both an estimate and");
  console.log("          actual hours, so his delta is the only meaningful one on the page.\n");
  console.log("WALKTHROUGH, in order:\n");
  console.log(`  1. /admin/payroll?period=${sel}`);
  console.log("     Exactly the seven rows above. NO download button — Dov is open. TWO notices:");
  console.log("     a red one (open punch, blocking) and an amber one (Gil's missing day, not).");
  ...
  console.log("  6. Gil's amber notice is STILL THERE and the file still built. That asymmetry");
  console.log("     is #638's whole decision — a missing day is checked, never a blocker.");
