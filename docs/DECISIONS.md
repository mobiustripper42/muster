...
- **Webhooks (`order.create`/`order.update`/`order.cancel`) are the future low-latency layer (Architecture C), deferred** to task 5.4c: they require a verification method (signature/secret — not yet documented from reachable Xola docs) and demote the poll to a reconcile backstop. The poll client (`getOrder` re-fetch) is exactly what the webhook receiver will reuse — 5.4b is its foundation, not throwaway. *(Independently corroborated: the sibling `crewbook` reached the same poll-then-webhook conclusion.)*
