...
**Status:** Built (Session 22). Originally `db/seed-pilot-crew.ts` seeded every crew member a far-future sentinel MMC expiry (`2099-12-31`); a real `mmcExpiry` overrides it per person as records are collected. **Since the pilot crew seed was retired** (2026-07-25; the no-PII-in-seeds rule now lives in `docs/RUNNING.md`) that script is gone and `db:crew add` (`src/crew/crew-cli.ts` → `PLACEHOLDER_MMC_EXPIRY`) is the sole owner of the sentinel — same value, same rationale.
