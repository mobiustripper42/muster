...
**Decision.** Close the gap with a **`db:crew` CLI** (`list` + `set <id> --email/--phone/--name`), and
**document the whole break-glass kit** as a runbook in `DEPLOY.md` — no admin UI. `db:crew` mirrors
`db:admin`: framework-free over the Repository port, unit-tested on the in-memory double, run against the
...
**Scope held / seams left.** No `db:crew add`/delete (crew lifecycle stays with seed/import). No per-crew
session revoke — crew sessions are stateless by design (#300); the global `SESSION_SECRET` rotation remains
...
**Revisit if:** admin/crew count outgrows a CLI, or a crew member ever needs creating outside seed/import
(then `db:crew add`, or a real UI).
