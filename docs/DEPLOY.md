...
| Var | Where it comes from | Used for |
|-----|---------------------|----------|
| `CREW_SELF_SERVE` | **you set it** — `1` or `true` to enable | **The crew code-login front door** (`app/lib/flags.ts`, DEC-081). **OFF by default** so `main` stays promotable — so production must set it explicitly or crew cannot sign in at all |
...
| `MESSAGING` | optional — off unless set; `1` or `true` to enable | Gates `/admin/messages`, `/crew/threads` and the doorbell (`app/lib/flags.ts`). **Never set it to `false`** — see the traps |
...
| Var | Where it comes from | Used for |
|-----|---------------------|----------|
| `RESERVATIONS` | **you set it** — `1` or `true` | The customer flow: `/book`, checkout, the manage page (`app/lib/flags.ts`, DEC-111). **OFF by default** so `main` stays promotable |
...
**3. `MESSAGING` is read two ways, and they disagree.** Every flag now accepts both `1` and `true`
through one helper (`flagOn`, `app/lib/flags.ts`, #736) — that half of this trap is gone, and
`RESERVATIONS=1` works. What remains is `MESSAGING` specifically: `messagingEnabled()` asks whether
it is **on**, but `app/lib/booking-confirmation.ts:31,103`, `app/lib/sold-out-notice.ts:20` and
...
while `messagingEnabled()` reads off either way. Set it to `1` or `true`, or leave it unset; never
`0` or `false`.
