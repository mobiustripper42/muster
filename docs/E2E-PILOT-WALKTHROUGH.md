...
| 2.4 | **No fills-by on the board (DEC-038):** each row's right column | Shows **only** the `Xd Xh to trip` countdown — **no** `fills by …` line. The deadline lives in the cockpit, not the board (DEC-038); a board row no longer means the automation has given up — within ~2 days a still-worked uncrewed shift boards too (DEC-065). Verified at 3.2, where the cockpit reads **`deadline …`** | ☐ pass ☐ fail |
...
| 2.7 | On the **Firkin** row, tap **↗ Nudge &lt;name&gt;** | Green *"Last action: nudged … — asked, not yet filled"* notice + a "watch it ↗" link to its cockpit. **The Firkin row stays on the board** — it's within ~2 days, so an uncrewed shift stays visible even with the ask in flight (DEC-065); its *System tried* trail now shows the nudge | ☐ pass ☐ fail |
...
| 7.2 | A **near-term** row **stays** after you Nudge/Lean | Within ~2 days an uncrewed shift stays on the board even with an ask in flight (DEC-065) — nudging no longer hides it; you keep sight of what's short near the dock. (A **far-off** shift still leaves while it's worked — see 7.5.) | ☐ yes ☐ confusing |
| 7.3 | A **`deadline … · overdue`** in the cockpit (not shown on the board — DEC-038) | The deadline lives in the cockpit, not the board (DEC-038). An uncrewed shift boards once the trip is within the deadline (DEC-065), so by the time it's a board row the fills-by is already past — overdue by construction; the live countdown carries the urgency instead | ☐ yes ☐ confusing |
