...
| 15.5 | **The sold-out notice and the residual-race alert read the row, not the charge.** Hard blocker on 15.7, and honestly labelled: today the notice works. `app/lib/sold-out-notice.ts:37-49` pulls the customer's name, email and phone out of `charge.metadata`; delete the metadata and the notice is addressed to nobody. `SPEC.md:1823` already names the row as the source. `ConfirmResult`'s `lost` carries no reservation and can. `db/paid-unbooked-dev.ts:134` breaks and `verify` runs `typecheck:db`. **Re-estimated 3 → 5 at build:** it also rewrote the notice copy (which falsely said "You have NOT been charged"), added an admin alert on every charge-and-refund, amended §2.8.7 twice, and had to extract the residual-race compensation to stay under the lint complexity ceiling | 5 | — | [ ] [#973](https://github.com/mobiustripper42/muster/issues/973) |
...
**Phase 15 total: 20 tasks, 83 points.** (Was 7 tasks / 29 points before the issue #966 audit; 81 before 15.5 was re-estimated at build.)
