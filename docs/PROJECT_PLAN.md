...
| # | Task | Effort | Notes |
|---|------|--------|-------|
| 11.0 | **Partition + `source` discriminator** — migration (`Event.source`, `Reservation.source`, backfill `'xola'`); importer guard: skip + itemize a Xola event on a Muster-owned vessel-day; Muster-owned-vessel-day config. Contract tests both adapters | 5 | **DEC-106** · lands on `main` (inert until a vessel-day is Muster-owned) · @architect gate · issue [#366](https://github.com/mobiustripper42/muster/issues/366) |
| 11.1 | **Availability read model** — remaining capacity per Muster-owned event (`COI max − Σ booked party sizes`, `source='muster'`); pure deriver + tests | 3 | additive, safe on `main` · issue [#367](https://github.com/mobiustripper42/muster/issues/367) |
| 11.2 | **Stripe charge/refund/deposit service** — `stripe` dep; **lift + audit charge + refund from the sibling `sailbook` project** (deposit + balance-link is the net-new piece); create-session server action, success/cancel routes, env/secrets (Drew's keys). Service-layer + adapter-tested | 8 | **DEC-107** · may be more complex than sized · requires `sailbook` in session scope · @architect gate · likely splits 11.2a lift/audit / 11.2b deposit+balance-link · issue [#368](https://github.com/mobiustripper42/muster/issues/368) |
| 11.3 | **Booking write + atomic capacity claim** — signature-verified idempotent `checkout.session.completed` webhook → writes Muster-native Event(if new)+Reservation under an atomic capacity guard; contract-tested both adapters | 5 | **DEC-109** (REQ-CLAIM-1 sibling) · the correctness task · @architect gate; split webhook-infra vs capacity-guard only if the diff balloons · issue [#369](https://github.com/mobiustripper42/muster/issues/369) |
| 11.4 | **Booking-link generation + confirmation emit** — generate the DEC-020 capability-URL for the reservation; emit confirmation with the link over email + SMS. Service-layer (copy polish + the manage *page* are P12) | 3 | **DEC-108** · the customer half of the capability-URL ("living link") family — internal name only, never in customer/crew copy · issue [#370](https://github.com/mobiustripper42/muster/issues/370) |
| 11.5 | **Waiver consent field (pilot)** — checkbox + linked terms + consent timestamp on the reservation | 2 | **DEC-110** · Drew/Spink legal-sufficiency flag · real provider integration is P12 · do **not** build a waiver subsystem · issue [#371](https://github.com/mobiustripper42/muster/issues/371) |
| 11.6 | **Throwaway booking harness** — the extremely-thin, unstyled `app/(public)` form + availability list that exercises the whole service layer to put one real booking through. **Explicitly disposable — replaced wholesale in P12** | 3 | not customer-quality; just enough to drive the exit gate · issue [#372](https://github.com/mobiustripper42/muster/issues/372) |
| 11.7 | **Manifest hinge verification** — confirm Muster-native reservations surface on the shift card per-event manifest with no write-back sheet (§2.6.3 / DEC-012 already source-agnostic) | 2 | **test, don't rebuild** · issue [#373](https://github.com/mobiustripper42/muster/issues/373) |
| 11.8 | **Go-live hardening + rollback runbook** — one real paid reservation end-to-end on the live boat/slot; flag flip; runbook (refunds-manual-in-Stripe, dispute-watch-in-Stripe, single-flip revert, ~5-booking manual rollback incl. Stripe-held money) | 3 | feature → `main` merge gate · issue [#374](https://github.com/mobiustripper42/muster/issues/374) |
