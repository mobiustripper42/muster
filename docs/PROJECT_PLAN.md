...
| 14.7 | **Drop `checkout_holds`.** Table, `CheckoutHold` (`entities.ts:727`), the issue #713 sweep; `holder-token.ts` moves to the row. 16 files reference the hold at planning. Issue #806's cap-per-token becomes a cap on live pending rows per token, inside the same transaction | 3 | #806 | [ ] [#918](https://github.com/mobiustripper42/muster/issues/918) |
