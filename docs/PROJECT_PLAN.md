...
outcome when the capture disagrees with the row. Old 15.2 and 15.4 are narrowed, not absorbed.
...
| 15.4 | **Freeze `amountDueNowCents` onto the invoice.** Prerequisite for 15.6 and 15.8 — the number a reprice sets and a capture is compared against. `create-departure-payment-intent.ts:238-240` computes it after the invoice is built and stores it nowhere, while the comment two lines up claims the charge is "rebuilt from the frozen invoice". No migration: `booking_invoice` is jsonb | 3 | — | [ ] [#972](https://github.com/mobiustripper42/muster/issues/972) |
...
| 15.8 | **Reuse one PaymentIntent per pending row — raise its amount instead of minting a second.** The #946 root cause. Stripe: *"If the checkout process is interrupted and resumes later, attempt to reuse the same PaymentIntent instead of creating a new one."* **After 15.7, not before** — Stripe *merges* metadata on update, so an update landing while 18 keys are still live must re-post `priceCents`/`extrasCents` exactly, and one omitted key is a silent cheap booking. `PaymentPort` gains a state read with an explicit `"unknown"` member; `recordCheckoutAttempt` widens to a null payment id. One unresolved sandbox question gates the branch — whether the amount can be raised on a `requires_payment_method` intent that has already been confirmed once | 8 | 12; §2.8.7 | [ ] [#976](https://github.com/mobiustripper42/muster/issues/976) |
...
**Four sub-items are declined with written arguments**, each in 15.14 or its task's PR rather than silently dropped: removing `payment_method_types: ["card"]` (§2.8.35 still says card-only, the SDK does not type the `allowed_payment_method_types` filter for Checkout even at 22.6.2, and one of the two sessions is spec'd for deletion); `integration_identifier` (a Dashboard grouping label with nobody to read it); bounding hosted-session expiry (24h is already the shortest defensible window for a link the operator texts by hand); and a `processed_events` table (every handler is already idempotent at the domain level, and a row written before the work completes turns a crashed invocation into an event skipped forever).

**Two things the audit could not establish, both settled in a sandbox before 15.8's branch is cut**, and neither guessed at: the live account's enabled payment methods and each webhook endpoint's `api_version` (`booking-webhook.ts:51-53` says it "rests on a Stripe dashboard nobody can read from the repo", issue #544); and whether Stripe.js refuses a confirm when the deferred Elements amount and the intent amount differ, plus what the customer reads when it does.
