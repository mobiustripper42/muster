...
| 14    | 8*       | 44     | — (DEC-S026) | — | — | **18.8 pts/wk** — 44 pts over 16.4d (08-25→09-10); companion 22.0 pts/active-week over 2 active weeks; re-est'd 2, drift **+3** (14.7 3→5 — 34 files referenced the hold, not 16; #946 2→3); **1 issue added mid-phase (3 pts, ~7%)** vs P12's 65%; 3 closed issues unpointed (#824, #825, #826 — all closed *by* planned tasks); 30 PRs in window; #806 unbundled and deferred, the §2.8.8 reaper cut from spec rather than built |
| 15    | 4*       | 73     | — (DEC-S026) | — | — | **43.6 pts/wk** — 73 pts over 11.7d (09-10→09-22); re-est'd **6**, drift **−8** — the largest negative drift of any phase, and every one of the six was scope found to be already built or not worth building (15.5 is the lone +5). 16 PRs from session 106; 54 merged in the window across both lanes. 20/20 planned tasks closed, **0 added mid-phase** |

`*` Phase 12's and Phase 14's session and PR counts are **window** counts, not the phases': Phase 13
completed inside Phase 12's span (2026-08-04) and lanes A and B ran concurrently throughout both. 47
session files and 237 merged PRs fall in Phase 12's window, 8 and 30 in Phase 14's; none of those
numbers feeds a rate.
...
| 3.2b | **Tier 2 escalation mechanism** — stall detection in `tick` + `escalate()` (widen-stub + direct-nudge); still `Filling`, no Eric | 5 | **[x]** SPEC §1.2 Tier 2. Builds on the ask loop + 3.1a + 3.2a. **DEC-024.** [#47](https://github.com/mobiustripper42/muster/issues/47) · PR #49 |
...
| 11.5 | **Waiver consent field (pilot)** — checkbox + linked terms + consent timestamp on the reservation | 2 | **DEC-110** · Drew/Eric legal-sufficiency flag · real provider integration is P12 · do **not** build a waiver subsystem |
...
provider + legal sufficiency — all Drew/Eric (DEC-107/103).
...
| 12.13 | **Waiver: decide the provider + approach (DEC-110)** — a **spike** to pick the e-waiver provider and integration shape (per-guest vs per-booking; hosted-form+webhook vs embed). **The actual integration re-pokers once decided.** Waivers are **mandatory** — only the decision is deferred, not the requirement | 3 | owner-gated (Drew/Eric) · integration build is a follow-on task · [#466](https://github.com/mobiustripper42/muster/issues/466) |
...
|---|------|-----|--------|--------|
| 14.1 | **The decision record.** One DEC for decisions 1 and 2 together: four states, nullable `eventId`, the pending row is the authority, `checkout_holds` goes; `supersedes: [DEC-109]`. Search at planning: `grep -rli "checkout_holds\|checkout hold" docs/decisions/` returned DEC-163, DEC-165 and the DEC-109 worksheet — nothing records the replacement, so new id. Settles the three open questions at `SPEC.md:2098-2119`: the window stays 15 and env-overridable; whether a pending row shows on the admin calendar; whether the sweeper is built at all. Those answers gate 14.3 and 14.8 | 2 | — | [x] [#912](https://github.com/mobiustripper42/muster/issues/912) |
| 14.2 | **The status union and the nullable column.** Migration on `reservations`; `entities.ts:615` gains `pending` and `expired`; `eventId` nullable; `Source` (`entities.ts:566`) gains `admin` now so 14.8's sweeper can branch on it — the surface that writes it is 16.1. The importer's own type at `import-reservations.ts:70` follows | 3 | 6 (half) | [x] [#913](https://github.com/mobiustripper42/muster/issues/913) |
| 14.3 | **Every reader becomes an allow-list.** Every `status !== "cancelled"` in the tree, `find-booking.ts:76` first — it would hand a manage link to an unpaid row the moment 14.2 lands. Crew manifest, §3.3 cancel, §3.4 disputes, refunds: none may be handed a pending or expired row. Admin surfaces each get 14.1's answer: purchases view stops rendering non-`booked` as **Cancelled**; integrity report gains a null-`eventId` branch; calendar, customers, at-risk board, block impact each say what they show. Same PR stack as 14.2, never after it | 5 | 24 | [x] [#914](https://github.com/mobiustripper42/muster/issues/914) |
| 14.4 | **The write.** "Book & pay" writes a `pending` row before any Stripe call — reserved time, party size, cookie token, frozen money, frozen hold minutes **and** trip time (DEC-161). Availability and the claim measure rival pending rows by **their own** frozen hold minutes, under the hull-day lock. The row alone removes the trip from `/book`. One write, one freeze — a coherent 8, not split | 8 | 2, 4, 20, 23; #825 | [x] [#915](https://github.com/mobiustripper42/muster/issues/915) |
| 14.5 | **Confirm flips the row.** `confirmReservation(paymentId)` finds the pending row by the payment id recorded on it, sets `booked` and `eventId`, materialises the Event. The reservation id stops being derived from the payment id (`write-booking.ts:35-38`). The money still comes from Stripe metadata until 15.1 — this task changes identity, not the freeze | 5 | 6 (other half); unblocks 12 | [x] [#916](https://github.com/mobiustripper42/muster/issues/916) |
| 14.6 | **Retry on the same row.** Declined card → same row, reserved time untouched, matched by the httpOnly cookie token and never by typed email or phone; the new payment id recorded alongside the first | 3 | 10 | [x] [#917](https://github.com/mobiustripper42/muster/issues/917) |
| 14.7 | **Drop `checkout_holds`.** Table, `CheckoutHold`, the issue #713 sweep; `holder-token.ts` moves to the row. 34 files referenced the hold at build, not the 16 estimated. `claimDepartureSlot` writes the pending row inside the fit-and-fallback loop, so the write CAS is the contention point and a lost write falls through to the next hull instead of reporting sold out. Re-estimated 3 → 5 with issue #806 unbundled: the cap-per-token is its own task, not a rider on a deletion | 5 | — | [x] [#918](https://github.com/mobiustripper42/muster/issues/918) |
| 14.8 | **Expiry, sweeper, reaper.** Every reader tests `pending AND reserved time + window > now`, branching on `source` first (`SPEC.md:1824-1828`). The sweeper — if 14.1 keeps it — never labels a row carrying a payment id. Reaper horizon chosen. Scripted abuse and real abandonment distinguishable in the data. "How many checkouts walked away last month, and how long did each hold a boat" is a query. Tests: abandon a checkout and assert no Event, customer, code or message exists (criterion 7, with the Stripe receipt-doc check the audit names); `payment_intent.payment_failed` acked and ignored (criterion 11) | 5 | 7, 8, 9, 11 | [x] [#919](https://github.com/mobiustripper42/muster/issues/919) |
| 14.9 | **Calendar and write refuse the same set.** The read path finds pending rows by **overlap**, not slot identity (issue #826). Already-departed refused on both sides (issue #824). One test asserts both sides refuse the same set of trips | 5 | 1, 5; #824, #826 | [x] [#920](https://github.com/mobiustripper42/muster/issues/920) |

| 14.10 | **Retry with a changed guest count reprices the charge but not the row.** Added during P14 retro — filed mid-phase from 14.7's `/security-review`, built in-phase. `appendPaymentIntentToPending` wrote three columns while the builder constructed a full row from the resubmitted form, so a corrected phone or a changed party size repriced Stripe and never reached the reservation. Renamed `recordCheckoutAttempt`; the customer's answers are re-stated, the world's facts stay frozen. Re-estimated 2 → 3 at spec | 3 | #946 | [x] [#946](https://github.com/mobiustripper42/muster/issues/946) |

**Phase 14 total: 10 tasks, 44 points shipped** (9 planned / 41 pts, plus issue #946 added mid-phase at 3). Originally pokered 39: 14.7 re-estimated 3 → 5 at build, issue #806 split out and deferred. (14.7 re-estimated 3 → 5 at build; issue #806 split out and unscheduled.)
...
**Re-planned 2026-09-10 from the Stripe-conformance audit (issue #966).** The phase was pokered at
7 tasks / 29 points against §2.8 alone. Auditing the integration against Stripe's own current
documentation — 31 agents across two passes, each finding adversarially verified — found six areas
where the code and Stripe's guidance disagree, and one defect nobody had specced. **29 → 81 points.**
Old 15.1 is the only row that disappears: it turned out to be four tasks, because "confirm reads our
row" needs the amount frozen first, the sold-out notice moved off metadata first, and a defined
outcome when the capture disagrees with the row. Old 15.2 is narrowed — its superseded-payment row moves to 15.10. Old 15.4 is unchanged, carried
forward as 15.17 with a note saying 15.6 does not absorb it.

The API choice is **settled and recorded, not revisited**: Muster stays on a raw PaymentIntent with
deferred-mount Elements, because embedded Checkout cannot render a card field before its session
exists and `/book` validates the card before it claims a hull. That is 15.1.

Tasks stack in this order. The two migrations are both on `reservations` and are **strictly
sequential** — 15.3 merges before 15.9's timestamp is generated, and they are never open together.

| # | Task | Est | Closes | Status |
|---|------|-----|--------|--------|
| 15.1 | **The decision record: the booking charge is a raw PaymentIntent.** Id resolved from `docs/decisions/` at write time — a draft exists on issue #966. DEC-134 chose inline Elements and was retired; §2.8.4a/§2.8.4b, the spec text that replaced it, name neither Elements nor Checkout nor PaymentIntents — so retirement deleted the reason while Stripe now leads with Checkout Sessions. The ordering is the holding: `/book` mounts the Element with no server object, then validates the card, claims the hull, mints the intent (`checkout-form.tsx:167-174`). Stripe documents that shape ("Collect payment details before creating an Intent") and lists Elements-over-PaymentIntents as ADVANCED INTEGRATION. `revisit_if` Stripe ships a deferred mount for elements-mode sessions | 2 | — | [x] [#969](https://github.com/mobiustripper42/muster/issues/969) |
| 15.2 | **Pin the API version; bump `stripe` to `~22.6.2`.** First, because every later task adds Stripe calls and event branches and must be written against a fixed wire version. Today `apiVersion` is omitted (`stripe-payment.ts:81`) so outbound calls send the SDK's compiled-in `2026-06-24.dahlia` — **not** the account default — and a floating `^22.3.1` is what can move it with no code diff. 22.6.2's pin is `2026-08-26.dahlia`, the current GA version. **Proof is the typechecker and nothing else**: adding the literal before the bump fails `TS2322`, and no runtime assertion can distinguish "we pass it" from "we omit it" once SDK and literal agree. Hand step: one booking through `/book` on dev — **not** to read the header, which the typechecker already pins, but to confirm a dependency bump on the only module that talks to Stripe did not break taking money (`e2e/book-checkout.spec.ts:21` stops before the card is confirmed). No Stripe sandbox: the header the literal claims cannot differ from the SDK's own constant once typecheck is green, so a live call would confirm nothing in doubt. **Scope grew at build:** 22.6.x added `OtherString` to `Stripe.Dispute.Status`, which made issue #723's `const unreachable: never = status` uncompilable — the guard was rebuilt as a type-level assertion over the union's NAMED members, with a permanent `@ts-expect-error` negative control. Estimate held at 3 | 3 | — | [x] [#970](https://github.com/mobiustripper42/muster/issues/970) |
| 15.3 | **The confirmation that never arrives.** The highest-severity defect in the phase and in no one's plan. `sendConfirmation` is gated `outcome === "booked"` (`booking-webhook.ts:463`) while `recordPayment` at `:404` is awaited **uncaught** after the commit — so a post-commit throw 500s, Stripe redelivers, `confirmPendingRow` resolves `already`, and the customer is charged, booked and never told, with nothing alerting. The file already fixed this exact shape for the crew tip twenty lines below (`:481-485`) and calls the confirmation's gate "legitimate" because DEC-122 has no other dedupe. Fix: a persisted `confirmation_sent_at` marker replaces the outcome gate | 5 | — | [x] [#971](https://github.com/mobiustripper42/muster/issues/971) |
| 15.4 | **Freeze `amountDueNowCents` onto the invoice.** Prerequisite for 15.6 and 15.8 — the number a reprice sets and a capture is compared against. `create-departure-payment-intent.ts:238-240` computes it after the invoice is built and stores it nowhere, while the comment at `:235-236` claims the charge is "rebuilt from the frozen invoice". No migration: `booking_invoice` is jsonb | 3 | — | [x] [#972](https://github.com/mobiustripper42/muster/issues/972) |
| 15.5 | **The sold-out notice and the residual-race alert read the row, not the charge.** Hard blocker on 15.7, and honestly labelled: today the notice works. `app/lib/sold-out-notice.ts:37-49` pulls the customer's name, email and phone out of `charge.metadata`; delete the metadata and the notice is addressed to nobody. `SPEC.md:1823` already names the row as the source. `ConfirmResult`'s `lost` carries no reservation and can. `db/paid-unbooked-dev.ts:134` breaks and `verify` runs `typecheck:db`. **Re-estimated 3 → 8 at build.** The issue named one surface; staging the real race in the app found the other two lying as well. It rewrote the notice copy (which falsely said "You have NOT been charged"), added an admin alert on every charge-and-refund, fixed `/book/success` telling the loser "You're booked!", stopped the refund reconciler texting RECONCILE MANUALLY about our own auto-refund, amended §2.8.7, and extracted the residual-race compensation to stay under the lint complexity ceiling | 8 | — | [x] [#973](https://github.com/mobiustripper42/muster/issues/973) |
| 15.6 | **Confirm reads the row's frozen invoice; a capture that disagrees is a named outcome.** DEC-164's ruling, active. `SPEC.md:2035`: *"No booking assembled from data Stripe hands back."* The mechanism is the deletion — `ConfirmCharge` loses `priceCents`/`extrasCents` and becomes `{ paymentIntentId }`, after which no parameter exists through which a Stripe number can reach the booking write. Three corrections found at spec time: `Payment.taxCents` is required so the "omit the carve-outs" plan is unbuildable and zeroes would make an underpaid booking read **paid**; the mismatch alert must be gated to `booked` or `/book/success` reloads re-alert every admin; and `no_invoice` cannot reuse the "no live pending reservation" alert body | 8 | #812; §2.8.5, §2.8.14 | [x] [#974](https://github.com/mobiustripper42/muster/issues/974) |
| 15.7 | **The booking charge sends one metadata key.** Delete 17 of 18 keys (`create-departure-payment-intent.ts:257-286`), keeping `purpose`. `SPEC.md:1744`: *"the booking charge sends no metadata at all."* Stripe: *"Don't include sensitive data in metadata, such as PII"* — we ship name, email and phone there. `description` and `receiptEmail` stay (both first-class fields, #679). `purpose` survives as DEC-134's double-write guard, which contradicts the literal spec line and needs one record with `amends_spec` on §2.8, **id resolved from `docs/decisions/` at write time** | 3 | §2.8.5 | [x] [#975](https://github.com/mobiustripper42/muster/issues/975) |
| 15.8 | **Reuse one PaymentIntent per pending row — raise its amount instead of minting a second.** The issue #946 root cause. Stripe: *"If the checkout process is interrupted and resumes later, attempt to reuse the same PaymentIntent instead of creating a new one."* **After 15.7, not before** — Stripe *merges* metadata on update, so an update landing while 18 keys are still live must re-post `priceCents`/`extrasCents` exactly, and one omitted key is a silent cheap booking. `PaymentPort` gains a state read with an explicit `"unknown"` member; `recordCheckoutAttempt` widens to a null payment id. One unresolved sandbox question gates the branch — whether the amount can be raised on a `requires_payment_method` intent that has already been confirmed once | 8 | 12; §2.8.7 | [x] [#976](https://github.com/mobiustripper42/muster/issues/976) |
| 15.9 | **Count checkout attempts on the row.** 15.8 falsifies a sentence already on an operator screen: `/admin/abandonment` says *"Card form counts payment attempts"* while `abandonment.ts:85` derives it from the id array's length — after reuse, two attempts render `1×`. Not dead, which is worse. The column earns a second job in 15.11. **Migration on `reservations`, after 15.3's and never open with it**; `reservationValues`' `?? null` idiom breaks a `not null default 0` | 5 | — | [x] [#977](https://github.com/mobiustripper42/muster/issues/977) |
| 15.10 | **Retire a superseded PaymentIntent instead of leaving it payable.** Two narrow cases survive 15.8: a remint because the old intent was in flight, and a booked row with other live ids. Cancel is permitted from `requires_payment_method`, `requires_confirmation`, `requires_action`, `requires_capture`. `SPEC.md:1822` defines today's outcome — *"refund it and tell the customer"* — and cancelling the losers is strictly better: no charge, no refund, no apology. Best-effort **after** the flip: a redelivery cancels an already-cancelled sibling and gets a 400, which must be a log line. `payment_intent.canceled` gets a named member; the Dashboard must subscribe | 5 | §2.8.7 | [x] [#978](https://github.com/mobiustripper42/muster/issues/978) |
| 15.11 | **Idempotency key on `paymentIntents.create`, keyed on the row id and the attempt ordinal.** What remains of the idempotency area after 15.8 subsumes most of it. The SDK already mints a retry key per POST (`maxNetworkRetries` defaults to 2), so a dropped connection cannot duplicate an intent today. `booking_${row.id}` — the obvious key — is **actively wrong** after 15.8: the row id is constant across attempts, so a remint either returns the dead intent or 400s. `booking_${row.id}_${attempts + 1}` dedupes two concurrent submits and lets a sequential retry mint. **No key on `update`** | 2 | — | [x] [#979](https://github.com/mobiustripper42/muster/issues/979) |
| 15.12 | **Gate hosted-session fulfilment on `payment_status`, and name the delayed-payment shapes.** `recordBalancePayment` and `recordPostGratuity` write `succeeded` unconditionally. Stripe: fulfil only when `payment_status` isn't `unpaid`, and handle `checkout.session.async_payment_succeeded`/`_failed`. **`payment_method_types: ["card"]` is NOT removed here or anywhere in this phase** — see the refusals below; removing the card lock before this gate exists would pay a crew tip four days before the ACH failed. Also gives `stripe-payment.ts` — 293 lines, zero tests — its first test file. **Carries the decision record for keeping the card lock** — a standing deviation from an explicit Stripe rule, so it is a live fork and not a paragraph; `revisit_if` the SDK types `allowed_payment_method_types` for Checkout. Also names `payment_intent.processing`, which today falls through `parseEvent` to `null` on a path with `automatic_payment_methods` enabled. Hand step: subscribe both async events, **without which this task is a regression** | 5 | — | [x] [#980](https://github.com/mobiustripper42/muster/issues/980) |
| 15.13 | **Carry the Stripe `event.id` through the payment port and log it.** Nothing in Muster ever sees one, so a 500 leaves a Workbench delivery record and a Vercel error line with nothing joining them. Name it `stripeEventId` — `eventId` means the departure, including in the same file. `console.error`, because that is what sheepdog reads in prod (`log-channel.ts:38-40`). `PaymentSucceeded` stays unchanged: `/book/success` has no event to take an id from | 3 | — | [x] [#981](https://github.com/mobiustripper42/muster/issues/981) |
| 15.14 | **One decision record: the webhook stays synchronous, and the endpoint is not IP-allowlisted.** Nothing executable changes. The 500 **is** the durability — Stripe redelivers for three days, and answering 2xx first converts a recoverable delay into a silent loss for no wall-clock gain, since `after()` runs inside the same max duration. Vercel request cancellation is opt-in and `vercel.json` does not set it, so the work completes and only the ack is lost. An allowlist is a Dashboard WAF rule invisible to the repo, and Stripe's IPs change on seven days' notice — a stale one 403s every delivery for three days. `revisit_if` §2.8.9's reconciler ships | 2 | — | [x] [#982](https://github.com/mobiustripper42/muster/issues/982) |
| 15.15 | **Late money — the two §2.8.7 rows that remain.** Narrowed by 15.10, not absorbed: cancelling losers makes the superseded-and-already-booked case rarer, and `SPEC.md:1822`'s refund path stays for rows booked before it ships and for a cancel that Stripe refuses. Still owed in full: success after the window with the boat taken → refund and tell the customer in one path, on the phone the reservation carries; and success after the window with the boat free → re-check under the lock and book it | 5 | 12; #668 | [x] [#983](https://github.com/mobiustripper42/muster/issues/983) |
| 15.16 | **The reconciler.** Primary: pending rows past their window with a payment id → ask Stripe → the same `confirmReservation`. Fallback: Stripe's undelivered-event feed — `src/adapters/stripe-payment.ts` has no event-listing method, so the port method comes first. A cron entry in `vercel.json` beside `tick` and `doorbell-tick`, on minutes not hours. Safe at any time, in any order, more than once. Runs under the pause. Named in 15.14's `revisit_if` twice | 8 | 15 | [x] [#984](https://github.com/mobiustripper42/muster/issues/984) |
| 15.17 | **The unmatched charge.** A throwing `alertPaidButUnbooked` still returns success to Stripe — one of 14 call sites guards today; the rest propagate to a 500 and a retry storm. The SMS names the amount. SMS unconfigured leaves one log line, and a test says so. Not absorbed by 15.6, which adds a mismatch alert but does not fix this one throwing | 3 | 19 | [x] [#985](https://github.com/mobiustripper42/muster/issues/985) |
| 15.18 | **Remove post-trip tipping.** `app/b/[code]`'s completed-state tip, `create-gratuity-checkout.ts`, and what the manage page says about it. §2.8.14: *"It ships today and has to be removed"*. This is also why the gratuity session's Stripe hygiene is not worth fixing — 15.12 gates it, 15.18 deletes it | 3 | §2.8.4b | [x] [#986](https://github.com/mobiustripper42/muster/issues/986) |
| 15.19 | **Delete the dead hosted-checkout path.** Issue #793 as filed. The balance path stays dormant per §2.8.10 — not this | 3 | #793 | [x] [#987](https://github.com/mobiustripper42/muster/issues/987) |
| 15.20 | **Tip pool "and no others".** One test that puts a disqualifying seat — non-required, unconfirmed, or unassigned — on a shift and asserts it receives nothing (`gratuity-payroll.ts:114`) | 2 | 22 | [x] [#988](https://github.com/mobiustripper42/muster/issues/988) |

**Phase 15 total: 20 tasks, 86 points.** (Was 7 tasks / 29 points before the issue #966 audit; 81 before 15.5 was re-estimated at build, 3 → 8.)

**If the window is short**, the spine that closes actual money defects is **15.3 → 15.4 → 15.6 → 15.8 → 15.10 = 29 points**. 15.5 and 15.7 are prerequisites for the PII deletion rather than fixes; 15.2, 15.11, 15.12, 15.13 and 15.14 are guards and hygiene.

**Four sub-items are declined with written arguments**, each in 15.14 or its task's PR rather than silently dropped: removing `payment_method_types: ["card"]` (§2.8.14 still says card-only, the SDK does not type the `allowed_payment_method_types` filter for Checkout even at 22.6.2, and one of the two sessions is spec'd for deletion); `integration_identifier` (a Dashboard grouping label with nobody to read it); bounding hosted-session expiry (24h is already the shortest defensible window for a link the operator texts by hand); and a `processed_events` table (every handler is already idempotent at the domain level, and a row written before the work completes turns a crashed invocation into an event skipped forever).

**Two things the audit could not establish, both settled in a sandbox before 15.8's branch is cut**, and neither guessed at: the live account's enabled payment methods and each webhook endpoint's `api_version` (`booking-webhook.ts:50-51` says it "rests on a Stripe dashboard nobody can read from the repo", issue #544); and whether Stripe.js refuses a confirm when the deferred Elements amount and the intent amount differ, plus what the customer reads when it does.
...
| # | Task | Est | Closes | Status |
|---|------|-----|--------|--------|
| 16.0 | **Pending write refuses a slot backed by an unbooked override Event.** Issue #945 as filed, pulled into the phase at the operator's call rather than left as 16.1's blocker. The availability read drops a muster Event at the identity being asked about; `savePendingIfHullFree` exempts nothing, so a per-departure price override makes that departure unsellable — and since 14.7 the loss falls through to the next fitting boat, so the symptom is *"the override silently moves customers to a different boat at a different price"* rather than a hard sold-out. Not live: nothing materializes an unbooked scheduled muster Event today. **16.1 is what arms it, which is why this goes first** | 3 | #945 | [ ] [#945](https://github.com/mobiustripper42/muster/issues/945) |
| 16.1 | **The operator books.** §2.10.6, DEC-162 and DEC-163 already recorded. **Book** beside **Hold** on the calendar click; a page collecting the same customer details as public checkout; a `pending` row with `source: admin` and no window; a payment link minted fresh on each ask, carrying no durable token; confirm through the same path as any sale. Overlap and capacity refuse; cutoff, season and grid pass; a block passes but says so. The purchases list shows it unpaid, with the link. Ended only by a person. The link's mechanism — a Stripe Payment Link or our own Elements page — is decided in the task spec. **Blocked on [#945](https://github.com/mobiustripper42/muster/issues/945)** if this task materializes an `Event` at a slot before a booking exists: the pending write refuses its own unbooked override Event, so that departure becomes unsellable to anyone | 8 | gap A; §2.10.6 | [ ] [#1069](https://github.com/mobiustripper42/muster/issues/1069) |
| 16.2 | **Booking cutoff — spec.** DEC, §2.8/§2.10 text, and `booking cutoff` in `docs/dictionary.yml` kept apart from *booking horizon* (`SPEC.md:347`) and the rejected crew lead-time cutoff (`:324`). An `app_settings` row with a code fallback that is not the value; computed, never stored; its own slot state; **no number in the spec** | 2 | gap B | [ ] [#1070](https://github.com/mobiustripper42/muster/issues/1070) |
| 16.3 | **Booking cutoff — build.** Both paths — `availability.ts` precedence and `candidateVessels` — so calendar and write agree. The customer sees *"too late to book online, call us"*. The admin passes it | 3 | gap B | [ ] [#1071](https://github.com/mobiustripper42/muster/issues/1071) |
| 16.4 | **Discounts — spec.** DEC: off the fare-plus-extras base; tax, tip and the service fee recompute on the discounted base; amount versus percent; frozen on the row like every §2.8.4a component. And the hard one: a comp-to-zero booking has no payment id to confirm by, so it needs a second, named entry into `confirmReservation` | 3 | gap C | [ ] [#1072](https://github.com/mobiustripper42/muster/issues/1072) |
| 16.5 | **Discounts — build, operator-applied.** Partial and full, from 16.1's booking page. Promo codes deferred | 8 | gap C | [ ] [#1073](https://github.com/mobiustripper42/muster/issues/1073) |
| 16.6 | **Audit trail — spec.** Issue #886 as filed | 3 | #886 | [x] [#1048](https://github.com/mobiustripper42/muster/issues/1048) — shipped by lane A before the phase opened |
| 16.7 | **Reservation pause.** Issue #864 — beside the crew-engine pause; a paused reservations surface refuses new checkouts so a settings edit never races a live one. The reconciler ignores it (§2.8.9) | 3 | #864 | [ ] [#864](https://github.com/mobiustripper42/muster/issues/864) |
| 16.8 | **Flex insurance as a boolean.** Issue #683 re-scoped to §2.8.4c: offered at checkout, frozen on the row, not taxed, not in the fee base, terms shown on the manage page. DEC-113 gets the correction §2.8.14 says it is owed | 5 | #683 | [ ] [#683](https://github.com/mobiustripper42/muster/issues/683) |
| 16.9 | **Self-service cancellation.** A Cancel action on the manage page that applies §2.8.4c itself — refund minus the $50 fee outside the window, nothing inside it; the window is 14 days, or 72 hours with flex, and flex never waives the fee (`SPEC.md:1712-1713`) — through the §3.3 refund path. *Changing* a booking stays a request to a human; §2.8.12's "request, not an action" paragraph is rewritten to name cancel as the exception. After 16.8 | 5 | §2.8.14 | [ ] [#1074](https://github.com/mobiustripper42/muster/issues/1074) |

**Phase 16 total: 9 tasks, 40 points.** — *unchanged at materialization (2026-09-22), but not the
same nine.* **16.6 was already shipped**: issue #886 closed complete on 09-20 and lane A built it as
issue #1048 (`src/domain/reservation-trail.ts`, `src/reservations/reservation-trail-view.ts`) on
09-22. Its 3 points are replaced by **16.0**, issue #945 pulled in from 16.1's blocker note so the
ordering does the work a `blocked` label would have. 16.7 and 16.8 reuse the issues the rows already
named (#864, #683); **#683's label was 3 against the row's 5** and was corrected to 5 at
materialization — the row says it was re-scoped to §2.8.4c and the label never moved.

**This phase has had no audit.** §2.10 has no acceptance criteria, and Phase 15's zero-adds record
was substantially bought by the Stripe-docs audit that opened it. Expect row premises to need
checking against current code before each branch is cut — 16.6 was the first one to fail that check
and it failed before the phase started.
