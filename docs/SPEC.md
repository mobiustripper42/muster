...
- **Cross-cutting** — notifications/push, passwordless auth, and the cancel-cascade + dispute-alert
...
- ~~The **customer portal** — sketch only, Tier 4, off-season 26/27. Builds last; not on the
  ...
  the real customer UI. One **cutover** (**DEC-126, 2026-07-17**): Xola sells until it, Muster after it,
  never both at once (DEC-149) — a one-time full import of Xola's reservations into Muster, after which
  Muster is the reservation source of truth (reversible: if a bug can't be fixed, cut back to Xola). 2026 reservations do **not**
...
> **a single cutover**, not the old "2027 switch": Xola sells until the **cutover** (DEC-126), Muster
> sells after it, and **the two never sell at the same time** (DEC-149, which retired the DEC-106
> owned-day partition that a simultaneous-selling stretch would have needed). The cutover is a **one-time
...
> drains naturally" wording; DEC-126 adds the flip, which does include a one-time migration.)* The ingest
> is the API pull, not a CSV (DEC-036/037/043).
...
- **2026 — coexistence.** Xola owns bookings/money/waivers ~~. A CSV export from Xola is imported~~
  ...
  **guides in Xola** (manual write-back from a Muster-emitted sheet) because the guest manifest still
  lives in Xola. Muster does **not** sell alongside Xola before the cutover (DEC-149); what survives of
  DEC-106 is the `source` discriminator on Event and Reservation.
- ~~**2027 — Muster takes bookings.** Shifts auto-form from Muster's own live feed; the CSV step
  ...
  2026, via a cutover, not a dated switch.** Until the cutover, Xola sells and Muster imports for
  crewing. At the **cutover** (DEC-126) Xola's reservations are imported once, the pull stops, and
...
### The record (fields)
- **Identity:** name; **phone** (SMS/push target — crew-app §1); email
  (optional secondary).
...
**not a 2027 event** — it is **Phase 11/12, now** (DEC-105). The two source-of-write paths are
**sequential, never concurrent** (DEC-149): the Xola import runs **until the cutover** (DEC-126),
which is one final full import of Xola's reservations, after which Muster is the reservation source of
truth, Muster's own writes carry `source` (DEC-106), and **the ongoing Xola pull stops**. This layer stays either way — that part held; only its *upstream* goes away.
...
<!-- amended-by-dec: generated by `npm run gen:decisions` — do not edit by hand -->
> **Amended by DEC-181 — the ask's link lands a signed-in crew member on Yes/No; a signed-out one signs in with the code first — 'no login' now means 'no password'**
<!-- /amended-by-dec -->
...
- ~~**Two buttons. ~3 seconds. No login, no navigate-to-respond.**~~ — **half met, half settled against.**
  **"No password" is real and shipped:** the SMS carries a plain link to the app (DEC-181 — no secret
  in any text); a signed-in crew member lands on Yes/No, a signed-out one signs in with the 6-digit
  code first (DEC-081). There is never a password step. **"No
  ...
  **Settled on operator input, 2026-07-27: there will never be reply-by-SMS — too many problems.** The
  ...
  needs the same strike.
- **Passwordless auth** (§3.2) — casual crew won't manage credentials; a forgotten
...
### States to render
- **The ask** (yes/no, answered in-app via the link in the notification — not from the notification itself; see
  §2.6.1).
...
### Actions
- **Accept / decline an ask** (in-app, via the link in the notification).
...
- **Session lapsed** when the ask's link is tapped → the code door, then the app — not a dead end (§3.2).

### Acceptance criteria
- [ ] ~~An ask is fully answerable (yes/no) from the push/SMS without opening or logging into the
      app.~~ — **REWRITTEN.** An ask is answerable in **two taps from the notification** for a signed-in
      crew member — tap the link, tap Yes or No — **with no password**; a signed-out one enters the
      6-digit code between the two (DEC-181). *(The "without opening" half is closed on operator input,
      2026-07-27: there will never be reply-by-SMS. DEC-030 chose a link with no inbound webhook and
      that stands.)*
...
<!-- amended-by-dec: generated by `npm run gen:decisions` — do not edit by hand -->
> **Amended by DEC-175 — the surface is no longer dark by default, and no longer coupled to the login door by one env var**
> **Amended by DEC-176 — the count reads "N you can claim" not "N open", and the page carries a second display-only section**
<!-- /amended-by-dec -->
...
>
> **Live everywhere.** `/crew/open`, the claim action and the hub entry were dark behind
> `CREW_SELF_SERVE`, coupled to the DEC-079/DEC-081 code-login front door by the same env var. The
> flag was turned on in production once email delivery was real, and deleted once its off-state
> existed nowhere — on in prod, on in e2e, so nothing ever ran the dark path.
...
ink (DEC-042) — ~~no live counts~~ a bare ~~`N open`~~ **`N you can claim`** row count for orientation
is fine, which is how the surface reads DEC-042's guardrail. Empty = normal, not an error.
*(Amended, DEC-176: the count is of seats **this viewer** can take, which is not the set of seats that
are open — the two differ whenever somebody isn't cleared for a boat.)*

**2.7.1a Other shifts (DEC-176).** Below the claim list, a **display-only** second section
over the same date range: every non-cancelled boat-day, with **vessel hue + vessel · first departure ·
trip count · who is aboard (name · role) · which roles are unfilled**. A boat carrying a claimable seat
appears in **both** sections. Nothing here is interactive, and it must not look interactive — claim rows
are cards with a chevron, team rows are flat. It exists because "nothing to claim" and "nothing is
happening" had the same words, and a fully-crewed fleet is the best possible state. **Carries no shift
state, no pax, and no way to tell an `Asked` seat from an untouched one** (DEC-008).
...
- [ ] The surface renders nothing and claims nothing for a viewer with no session, and nothing a
      viewer is not eligible for. *(Replaces the `CREW_SELF_SERVE`-off criterion, retired by DEC-175
      — what gates this surface is the session and the §1.3 eligibility rules, not a flag.)*

---

## 2.8 Booking & Payment — the customer buys a trip
...
`pending`, and it is the thing that occupies the trip. It carries the instant it was written —
its **reserved time** — and the payment window is a setting, not a column. Expiry is that instant
...
schedule rather than from stored rows, so a pending reservation removes its trip from the
...
**2.8.3 What a reservation occupies is the hull for the trip's hold minutes.**
...
is time no other trip can have. **Both numbers are per-offering configuration and they differ
...
trip that starts while the first boat is still tying up.

That failure is invisible on a single offering at even spacing, because the next published departure
is past the error anyway and off-grid times are refused. It appears the day a second offering runs on
...
trip currently being sold is wrong in both directions.
...
3. **On submit, holding the hull-day lock:** refuse anything off-grid, outside the offering's season,
   ...
   **freeze the money, the hold minutes and the trip time onto it**.
...
**"Freeze" means write the computed values onto the reservation and never recalculate them.** Every
later reader — the confirmation, the receipt, the balance, a refund — reads those stored values.
Nothing recomputes from live settings, because a tax rate, a service fee, a price or a trip time can
change between the quote and the payment, and the customer must get what they were shown.

**The frozen money is one value on the reservation, the `booking_invoice`.** Not a column per
component. The set grows — add-ons are a list of unknown length and a discount is coming — and a
shape that has to gain a column every time §2.8.4a gains a row is a migration standing in for a
sentence. Nested values are stored as JSON here already, which is the house posture rather than a
new idea.

**Anything with a time in it is a column, never inside the invoice**, because those get queried and
a value buried in JSON does not. The freeze instant is one of them, and it is not a new field: it is
the reserved time from 2.8.1, written in the same locked step as the money.

**What the invoice carries.** Every component in 2.8.4a as integer cents, **and the rate behind each
one** — the tax rate, the service-fee rate, the gratuity tier. A receipt that states eight dollars of
...
**And this is why the payment carries nothing.** With the invoice on our own row there is nothing the
booking needs back from the payment object but its id and its status (2.8.5).
...
**Also frozen: both durations.** The **hold minutes**, because 2.8.3 measures occupancy with them and
an offering can be edited mid-window. And the **trip time**, because that is the number the customer
was quoted — "1h 40min on the water" — and it must not move under them either.

Turnaround is not stored. It is hold minutes minus trip time, and storing a third number invites the
three of them to disagree.

**2.8.4b Where the tip goes.**

The tip pool for a trip is **split equally among the crew who worked it** — the confirmed holders
...
offering, the trip, the party size and who booked. **So the booking charge sends no metadata at
...
Every row below describes a **public checkout**. An operator's booking has no window, so the three
rows that turn on one — abandonment, and a payment landing after expiry — do not apply to it
(§2.10.6).

| What happens | What we do |
|---|---|
| Card declined, customer retries | Same reservation, same boat, **reserved time not touched** — otherwise a session parks a hull indefinitely by resubmitting. A new payment against the same row, its id recorded alongside the first (2.8.5). |
...
| Payment succeeds after the window expired, and the boat has been taken | Refund the full amount automatically — no operator action required — and tell the customer on the channel a confirmation would have used (the phone on the reservation; phone is required, email is not). The message says the trip sold out while they were paying, and says plainly that they **were** charged and then refunded: the money really was captured, the refund takes days to settle, and a message claiming they were not charged is contradicted by their own statement for that whole window. It does not state the amount — the figure adds nothing to a message whose point is that all of it is coming back. **The refund and the message are one path:** a refund nobody was told about reads as a silent failed payment. **And the office is told every time**, with no action asked for: this is a customer charged for a trip they did not get, and the count of how often it happens is the only evidence that decides whether the payment flow moves to separate authorization and capture. |
...
time, and filters the calendar and the trip list to boats that can take the party.
...
address settling behind it. Every day cell, trip row and month pager has the count baked
...
- [ ] Pressing "Book & pay" writes a `pending` reservation **before** any call to Stripe, and that row
      alone removes the trip from `/book`.
- [ ] **Two concurrent bookings on one hull at overlapping times — 13:30 and 14:00, not the same
      trip — produce exactly one sale.** Run it against real Postgres, repeatedly, and assert the
...
- [ ] Two offerings with different hold minutes on one boat block each other by **their own**
      ...
      a trip booked at 13:30 refuses a rival at 15:15, which measuring in trip time would sell.
- [ ] The calendar and the write refuse the *same* set of trips.
...
- [ ] Confirming a trip whose slot was previously booked and then cancelled succeeds, and the
...
- [ ] Editing the offering's price, hold minutes, trip time or schedule while a reservation is pending changes
...
- **How long is the payment window?** Fifteen minutes is a guess carried over from Sailbook and from a
  ...
  Because 2.8.1 stores the reserved time and not the expiry, changing that setting moves the deadline
...
The day view is a **grid** — boats as columns, trips as blocks down the day. Week and month
...
Muster does **not** write a row for every trip it could sell. The schedule is a rule and
...
both durations are written onto the booking, so a later price change cannot reach back — but
there is also no operator surface that edits a booked trip's time or capacity at all. The
...
- **a per-trip override** — editing one trip's time, price or capacity without
...
**2.10.3 Blackout is blocks, not per-trip toggles.**
...
A trip is **Open or Sold**. It is never a seat count, because BrewBoat sells the whole boat
...
**2.10.6 The operator books.**

Someone rings up. The operator takes the booking. Until this, they could not — every reservation
in the system arrived from the public site or from Xola, and there was no way to write one by hand
at any notice, at any price.

**It starts from the calendar click that already exists.** An open slot offers **Hold** today
(§2.10.3 — that writes a block). It gains **Book** beside it, and that opens a page collecting the
same customer details the public checkout collects.

**The two surfaces compute the money from one model.** The same trip never quotes two totals,
for the same reason §2.8.12 gives about the manage page and the admin pane. Everything in §2.8.4a
applies unchanged — the components, the freeze, the tax and fee bases.

**No card is read over the phone, ever.** The operator does not type a card number into Muster and
there is no field for one. Card numbers never reaching our servers is what puts this product in the
lightest compliance tier, and an admin card field would end that — for the whole product, not just
this screen.

**So the operator books, and the customer pays a link.** The reservation is written and holds the
boat immediately; a payment link goes to the customer; the booking confirms when they pay, through
...
either cancels it or gives them longer.

That is the whole mechanism and it is deliberate. One operator, a few phone bookings, and a process
someone runs by hand beats a timer built for a volume that does not exist. Revisit it when this is
happening daily.
...
**2.10.8 What this surface is NOT.**
...
**And no way for the operator to take money directly.** No cash, no "already paid elsewhere", no
card terminal, no card field. Every payment in the system is a Stripe payment, which is also what
keeps §3.3's refund path working — money that did not arrive through Stripe cannot be sent back
through it.
...
<!-- amended-by-dec: generated by `npm run gen:decisions` — do not edit by hand -->
> **Amended by DEC-181 — one passwordless entry, the 6-digit code; action links are plain deep-links, never a login**
<!-- /amended-by-dec -->
...
- **The Xola API bolt-on.** A live API integration is a maintained dependency on a system with an
  ...
  > alternative to the CSV path, it *is* the path. And the adapter does **not** die "in 2027": the pull
  > **stops at the DEC-126 cutover** (a one-time full import of Xola's
