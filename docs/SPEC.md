...
> **Amended by DEC-145 — the Tier-1 broadcast's losing recipients are retired as `withdrawn` when the seat fills, and carry no reliability event — the tier description implied the losers simply went stale**
<!-- /amended-by-dec -->
...
<!-- amended-by-dec: generated by `npm run gen:decisions` — do not edit by hand -->
> **Amended by DEC-144 — `self_claim` joins the Commitment event list, and `shift_completed` finally has a producer — the tick's completion sweep. Both were declared loggable from day one and neither was ever emitted.**
> **Amended by DEC-145 — `ask_ignored` is only for genuine silence; an ask retired because its seat filled is `withdrawn` and scores nothing**
<!-- /amended-by-dec -->
