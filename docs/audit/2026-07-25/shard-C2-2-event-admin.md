...
`browse.test.ts:34`. **No caller** (C2.2-4) — there is no admin surface on which Eric can open an
...
**OQ-1 — "~~Merge rule for manual entries vs re-import~~ — RESOLVED by DEC-043, not by Eric/Drew."**
...
**One live operator-facing defect (C2.2-3).** `/admin/import` tells Eric the pull "runs automatically
...
Eric may already text people — but unlike C2.1-8 this one has a wrong comment and an absent test
