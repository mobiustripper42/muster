...
(gates)"*, `:73` *"adds a supernumerary seat (non-gating) that survives a re-form"*. Reachable UI:
`components/assignment/manning-section.tsx:163,172`.
**Second half, NOT MET — and the gap is asserted as fact in three places.** See C2.3-6: no code
anywhere subtracts a supernumerary seat from `coiMaxPax` or from `Event.capacity`; the only capacity
predicate in the repo is the customer-side `canBook`, whose model is a **whole-boat mutex, not a seat
count** (`availability.ts:5-17`), and Xola-sourced bookings are never capacity-checked by Muster at all.
The operator UI states the rule as copy — "takes a pax slot, doesn't gate" (`manning-section.tsx:72`) —
next to a number that never changes. `BUILDER-RECONCILIATION.md:44-45` flagged exactly this a year of
phases ago and it did not ship with the bundle it was scheduled into.
...
| The **build → review** reframe survives DEC-082 — "shifts still form continuously and Spink still adjusts them — only the *commit* step is gone" | `SPEC.md:616-618`, `:622-626` | Accurate. Continuous formation (`xola-pull.ts:182`), adjustment (split/merge/override), no commit step anywhere. `USER_STORIES.md:24-25` SP-5 carries the same reframe and is not struck |
