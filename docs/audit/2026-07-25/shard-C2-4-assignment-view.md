...
| "In the autonomous posture the system performs broadcast → rank → confirm on its own; **these actions are Spink's manual equivalents for taking over**" | `SPEC.md:802-803` | True, and it is the load-bearing sentence behind DEC-027 §2's emergent-pause finding: the manual actions ride the *same* rails (`lean`/`assignPerson`/`confirmSeat`) rather than a parallel path, which is why a manual placement always leaves a live ask the autonomous tier then refuses to fight. (The "broadcast" verb is C2.4-2's problem, not this sentence's) |
