...
amends:
  - id: DEC-016
    relation: amends
    scope: "the vessel-from-product collapse only. The worked-example leg is live authority — cited by SPEC §Fleet for per-vessel manning counts"
  - id: DEC-037
    relation: supersedes
    scope: "the xlsx-first half is retired; the API Land adapter shipped"
  - id: DEC-036
    relation: amends
    scope: "the planned `fetchEvents` half is now the primary adapter; the xlsx upload is retired"
  - id: DEC-018
    relation: amends
    scope: "quarantine keys off `resource.id`"
  - id: DEC-029
    relation: amends
    scope: "`vesselId` joins the material set"
---
