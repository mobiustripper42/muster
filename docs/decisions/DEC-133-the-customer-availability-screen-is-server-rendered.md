...
- Revised by DEC-156 — the accepted wrinkle only — guest count now lives in the URL, and the order is guests → date → time. The server-rendering-default posture and the single-island boundary stand.
