...
- **Nothing false, ever.** No call-time token when the earliest departure did not move (a cancelled *late* trip leaves it alone). No call-time token when the watermark is unknown. Trips are **netted**, because `+1 trip, -1 trip` on a swap is noise and `+0` would be a lie — a pure swap moves the clock, and that token covers it.
