...
| **Dashboard** (day / week / month / list / equipment views) | Event calendar, filters by Product / Equipment / Guide, available-vs-reserved counts | **Already exists** — this is `/admin/shifts` in a different skin. |
| **Event detail pane** | Guide + Manage Guides · Equipment (Brew 4 ×1) · Capacity (0 available / 12 reserved) · Message, Change Arrival, Cancel Reservations, Email/Export/Print Roster · Event Notes · Guests table | **Mostly exists.** Guide = the crew engine. Equipment = vessel. Capacity = `Event.capacity` (DEC-112). Needs reservation *actions* added — not a rebuild. |
