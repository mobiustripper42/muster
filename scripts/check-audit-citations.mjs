...
const CITATIONS = [
  ...
  ['src/adapters/postgres-repository.ts', 1891, 'from checkout_holds where expires_at > $1'],
  ['src/adapters/in-memory-repository.ts', 812, 'h.expiresAt > asOf'],
  ['src/adapters/repository-contract.ts', 1305, 'listLiveCheckoutHolds("2026-07-01T12:15:00.000Z")).toHaveLength(0)'],
  ['src/adapters/repository-contract.ts', 1306, 'listLiveCheckoutHolds("2026-07-01T12:14:59.999Z")).toHaveLength(1)'],
  ['src/adapters/repository-contract.ts', 1309, 'sweeps EVERY expired hold, not just its own slot'],
  ['src/adapters/repository-contract.ts', 1331, 'never touches a LIVE hold on another slot'],
  ...
  ['src/adapters/postgres-repository.ts', 1844, "delete from checkout_holds where source='muster' and expires_at <= $1"],
  ['src/adapters/postgres-repository.ts', 1837, 'grew the table without bound'],
  ...
  ['src/adapters/postgres-repository.ts', 1658, 'select 1 from reservations where id=$1'],
  ['src/adapters/postgres-repository.ts', 1662, 'won.rowCount === 1'],
  ...
  ['src/adapters/postgres-repository.ts', 1590, 'on conflict do nothing'],
  ...
  ['src/adapters/postgres-repository.ts', 1388, 'on conflict (id) do nothing'],
  ...
  ['src/adapters/postgres-repository.ts', 1603, 'RESURRECT a cancelled slot (#616)'],
  ['src/adapters/postgres-repository.ts', 1625, "set status = 'scheduled', capacity = $4, price = $5, duration_minutes = $6"],
  ['src/adapters/postgres-repository.ts', 1627, "and source='muster' and status='cancelled'"],
  ['src/adapters/postgres-repository.ts', 1617, 'of the booking, and the candidate never carries one'],
  ['src/adapters/postgres-repository.ts', 1621, 'a predicate that could also match a LIVE slot'],
  ['src/adapters/postgres-repository.ts', 1645, 'slot un-materializable (e.g. cancelled) — no oversell'],
  ...
  ['src/adapters/repository-contract.ts', 1816, 're-freezes price and duration IDENTICALLY on both adapters (#616)'],
  ['src/adapters/repository-contract.ts', 1819, "kept the DEAD booking's numbers"],
  ['src/adapters/repository-contract.ts', 1867, 'expect(revived?.capacity).toBe(8)'],
  ['src/adapters/repository-contract.ts', 1868, 'expect(revived?.price).toBeUndefined()'],
  ...
  ['src/reservations/cancel-reservation.ts', 162, 'const cancelledEvent = await deps.repo.cancelEventIfUnclaimed(eventId);'],
  ['src/adapters/repository-contract.ts', 1658, '// ── Gratuity (DEC-124, 12.3)'],
  ...
  ['src/adapters/postgres-repository.ts', 1511, 'async saveBookingIfSlotFree('],
