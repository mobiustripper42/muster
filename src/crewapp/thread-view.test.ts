...
describe("buildThreadView — operator (admin) cross-visibility (#118, DEC-072)", () => {
  const OP: Subject = { kind: "admin", id: "eric" };
  ...
  it("reads ANY thread; mine = the office's own admin messages, not the handle", async () => {
    ...
    await repo.saveMessage(
      msg({ id: asId<"MessageId">("o1"), threadId: t, senderId: "crew-eric-stoffer", senderKind: "admin", body: "from office", createdAt: "2026-07-01T07:01:00.000Z" }),
    );
