...
describe("recoverBookingLink", () => {
  ...
  it("sends the manage link to the contact on file when everything matches", async () => {
    await recoverBookingLink(deps(), rows, { contact: "marcus@example.com", lastName: "Webb" });
  ...
  it("RETURNS THE SAME THING on a miss as on a hit — the whole point", async () => {
    ...
    const hit = await recoverBookingLink(deps(), rows, {
      contact: "marcus@example.com",
      lastName: "Webb",
    });
    const missName = await recoverBookingLink(deps(), rows, {
      contact: "marcus@example.com",
      lastName: "Nguyen",
    });
    const missContact = await recoverBookingLink(deps(), rows, {
  ...
  it("swallows a channel failure — an error would itself be a signal", async () => {
    ...
    await expect(
      recoverBookingLink({ ...deps(), email: broken, sms: broken }, rows, {
        contact: "marcus@example.com",
        lastName: "Webb",
      }),
  ...
  it("swallows a repository failure too, and reports it out of band", async () => {
    ...
    await expect(
      recoverBookingLink({ ...deps(), repo: brokenRepo, onFailure }, rows, {
        contact: "marcus@example.com",
        lastName: "Webb",
      }),
    ).resolves.toBeUndefined();
  ...
  describe("the throttle", () => {
    ...
    it("bounds the NO-MATCH path too — the one an abuser uses", async () => {
      ...
      await recoverBookingLink(deps(), rows, { contact: "marcus@example.com", lastName: "Nguyen" });
    ...
    it("throttles by canonical contact, so respelling a phone is not a fresh attempt", async () => {
      await recoverBookingLink(deps(), rows, { contact: "+1 216 555 0148", lastName: "Webb" });
      await recoverBookingLink(deps(), rows, { contact: "(216) 555-0148", lastName: "Webb" });
    ...
    it("does not throttle a different contact", async () => {
      ...
      await recoverBookingLink(deps(), rows, { contact: "+12165550148", lastName: "Webb" });
